...
def predict_intent(vectorizer, clf, input_text):
    input_text = vectorizer.transform([input_text])
    return clf.predict(input_text)[0]
...
def save_chat_log(user_input, response, timestamp):
    with open('chat_log.csv', 'a', newline='', encoding='utf-8') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow([user_input, response, timestamp])

def main():
    st.title("Chatbot for Gamers")
    ...
    if choice == "Home":
        ...
        if user_input:
            response = get_response(intents, predict_intent(vectorizer, clf, user_input))
            st.text_area("Chatbot:", value=response, height=120)

            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            save_chat_log(user_input, response, timestamp)
        ...
        st.write("""
        The dataset used in this project is a collection of labeled intents and entities. The data is stored in a list.
